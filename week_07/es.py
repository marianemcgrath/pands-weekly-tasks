...
try:                                                                    # We'll try to try/ except, assuming the user might make a mistake, so we might have to handle errors
    ...
    with open (moby_dick, "r", encoding = "utf-8") as file:             # Now, we open the text file, we'll read the file, then use .lower() to turn all letters into lowercase          
        ...
        count = file.read().lower().count('e')                          # Source: https://realpython.com/working-with-files-in-python/ (Working With Files in Python)
                                                                        # Source: DeepSeek AI (Prompt: Add to my code, a simple function to count both upper and lower cases of the letter 'e')
    ...
    print("Error: Could not read file")                                      # The program issues a general message
                                                                             # Source: Matthes, E. (2023) Python Crash Course (3rd edition). San Francisco, US: No Starch Press -- Chapter 11
...
# Source: https://docs.python.org/3/library/os.html#files-and-directories 
# Source: https://www.youtube.com/watch?v=bnVf5IyqEhw